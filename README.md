...
| SSO Logout Method        | The logout method to use for SSO. Choices are `Default` or `Custom URL`. `Default` uses the standard WorkOS SSO logout flow. If `Custom URL` is selected, the `Custom Logout URL` field must be set, and the user will be redirected to that URL upon logout.                                                                                           |
| Custom Logout URL        | The URL to redirect the user to upon logout, used when `SSO Logout Method` is set to `Custom URL`. Required in that case.                                                                                                                                                                                                                               |
...
| `WORKOS_SANDBOX_CLIENT_ID`            | `None`                                 | If provided this allows for a second set of credenitals that can be selected in LoginRule                                                                                                                                                            |
| `WORKOS_SANDBOX_API_KEY`              | `None`                                 | If provided this allows for a second set of credentials that can be selected in LoginRule                                                                                                                                                                              |
